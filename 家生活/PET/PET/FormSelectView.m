...
- (void)initScrollviewWithTitles:(NSArray *)titles lineArray:(NSMutableArray *)lineArray ImageNameArray:(NSMutableArray *)imageNameArray{
    ...
    selectView=[[UIView alloc]initWithFrame:CGRectMake(0, self.frame.size.height-10, self.frame.size.width/titles.count, 10)];
    [self addSubview:selectView];
    UILabel *selcetLable=[[UILabel alloc]init];
    selcetLable.backgroundColor=RGBA(153, 102, 51,1);
    selcetLable.layer.cornerRadius = 5;
    
    selcetLable.layer.masksToBounds = YES;
    selcetLable.frame=CGRectMake(selectView.frame.size.width/2-5,0,8,8);
    [selectView addSubview:selcetLable];
}
...
//    [self addSubview:view];
    ...
    [button setTitleColor:RGBA(153, 102, 51, 1) forState:UIControlStateNormal];
...
//    scrollViewContent.contentOffset=;
    [scrollViewContent setContentOffset:CGPointMake((button.tag-1000)*self.superview.frame.size.width, 0) animated:YES];
